...
def process_and_save_data():
    ...
    # 读取训练数据
    print("正在加载训练数据...")
    train_data = pd.read_csv(str(Paths.Data.used_car_train), sep=' ', encoding='utf-8')
    ...
    # 保存处理后的数据
    print("\n保存处理后的数据...")
    joblib.dump(X_train, str(Paths.Data.x_train))
    joblib.dump(X_val, str(Paths.Data.x_val))
    joblib.dump(y_train, str(Paths.Data.y_train))
    joblib.dump(y_val, str(Paths.Data.y_val))
    joblib.dump(label_encoders, str(Paths.Data.label_encoders))

    print("数据预处理完成！处理后的数据已保存到 processed 目录")
...
def process_test_data(label_encoders):
    ...
    print("\n正在处理测试数据...")
    # 读取测试数据
    test_data = pd.read_csv(str(Paths.Data.used_car_testB), sep=' ', encoding='utf-8')
    ...
    # 保存处理后的测试数据
    joblib.dump(test_data, str(Paths.Data.test_data))
    joblib.dump(sale_ids, str(Paths.Data.sale_ids))
